...
function PyriteOreModel({ isHovered }: { isHovered: boolean }) {
  const { scene } = useGLTF('/pyritechalcopyrite_ore/scene.gltf')
  const meshRef = useRef<THREE.Group>(null)

  // Debug: Log when model loads
  console.log('PyriteOreModel: Model loaded', scene)
...
export default function PyriteOre3D() {
  ...
  return (
    ...
    >
      {/* Gradient background */}
      <div className="absolute inset-0 bg-gradient-to-br from-yellow-50 via-amber-100 to-orange-200 opacity-80" />
      
      {/* Subtle pattern overlay */}
      <div className="absolute inset-0 opacity-10">
        <div className="w-full h-full bg-[radial-gradient(circle_at_50%_50%,rgba(255,215,0,0.3),transparent_50%)]" />
      </div>
      ...
      >
        ...
        <Suspense fallback={
          ...
          <PyriteOreModel isHovered={isHovered} />
        </Suspense>
        <OrbitControls 
          enableZoom={false} 
          enablePan={false}
...
// Preload the model
useGLTF.preload('/pyritechalcopyrite_ore/scene.gltf')
