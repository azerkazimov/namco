...
function BorniteOreModel({ isHovered }: { isHovered: boolean }) {
  const { scene } = useGLTF('/bornite_chalcopyrite_and_covelite_207_7-19/scene.gltf')
...
export default function BorniteOre3D() {
  ...
  return (
    ...
    >
      {/* Gradient background */}
      <div className="absolute inset-0 bg-gradient-to-br from-purple-50 via-indigo-100 to-blue-200 opacity-80" />
      
      {/* Subtle pattern overlay */}
      <div className="absolute inset-0 opacity-10">
        <div className="w-full h-full bg-[radial-gradient(circle_at_50%_50%,rgba(147,51,234,0.3),transparent_50%)]" />
      </div>
      ...
      >
        ...
        <BorniteOreModel isHovered={isHovered} />
        <OrbitControls 
          enableZoom={false} 
          enablePan={false}
          autoRotate={false}
...
// Preload the model
useGLTF.preload('/bornite_chalcopyrite_and_covelite_207_7-19/scene.gltf')
