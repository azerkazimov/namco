...
function ChalcopyriteOreModel({ isHovered }: { isHovered: boolean }) {
  const { scene } = useGLTF('/chalcopyrite/scene.gltf')
  const meshRef = useRef<THREE.Group>(null)

  // Debug: Log when model loads
  console.log('ChalcopyriteOreModel: Model loaded', scene)
...
export default function ChalcopyriteOre3D() {
  ...
  return (
    ...
    >
      {/* Gradient background */}
      <div className="absolute inset-0 bg-gradient-to-br from-amber-50 via-orange-100 to-yellow-200 opacity-80" />
      
      {/* Subtle pattern overlay */}
      <div className="absolute inset-0 opacity-10">
        <div className="w-full h-full bg-[radial-gradient(circle_at_50%_50%,rgba(255,165,0,0.3),transparent_50%)]" />
      </div>
      ...
      >
        ...
        <Suspense fallback={
          ...
          <ChalcopyriteOreModel isHovered={isHovered} />
        </Suspense>
        <OrbitControls 
          enableZoom={false} 
          enablePan={false}
...
// Preload the model
useGLTF.preload('/chalcopyrite/scene.gltf')
